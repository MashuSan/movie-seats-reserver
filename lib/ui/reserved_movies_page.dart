...
class ReservedMoviePage extends StatefulWidget {
  const ReservedMoviePage({Key? key}) : super(key: key);
  ...
  @override
  State<ReservedMoviePage> createState() => _ReservedMoviePageState();
}

class _ReservedMoviePageState extends State<ReservedMoviePage> {
  ...
  void initState() {
    ...
    _favoriteData = Future.wait([
      getReservedMovies().then((ids) => _provider
          .fetchTrendingMovies()
  ...
  Future<List<String>> getReservedMovies() async {
    final userId = FirebaseAuth.instance.currentUser!.uid;
    final snapshot = await FirebaseFirestore.instance
        .collection('movies')
        //.where((x) => x['reservedBy'])
        .get();
  print(snapshot.size);
